...
import configConverter from "../configConverter.json";
...
import {
  ...
  CURRENCY,
  CURRENCIES,
} from "../configConverter.json";

const URL_API =
  "https://api.currencyapi.com/v3/latest?apikey=CAFavxRfTumMQbpAjhdXc828owWNTHOtlp2pH7gx&base_currency=KZT"; /*&currencies=USD,RUB,EUR";*/
const db = {
  data: {
...
export default function Converter() {
  ...
  //     value = value.toFixed(2);
  //   }
  //   setDataBase((prev) => [...prev, { id, name, value }]);
  ...
  const getMoney = () => {
    ...
    for (const elem in db.data) {
      // makeDataBase(db.data[elem].code, db.data[elem].value);
      // console.log(db.data[elem].code+":", db.data[elem].value);
      ...
      fakeDb.push({ id, name, value, date, time });
    }
    // console.log(fakeDb);
    setDataBase(fakeDb);
  };
  ...
  async function getCash() {
    // const res = await axios.get(`${ENDPOINT}?access_key=${API_KEY}&base=KZT&symbols=USD,RUB,EUR`);
    // const response = await fetch(`${ENDPOINT}?apikey=${API_KEY}&value=1000&base_currency=${CURRENCY}&currencies=${[...CURRENCIES]}`);
  ...
  return (
    <View style={{ paddingHorizontal: 20, backgroundColor: "black" }}>
      ...
      <FlatList
        style={styles.main}
        ListHeaderComponent={<ConverterHeader />}
        data={dataBase}
